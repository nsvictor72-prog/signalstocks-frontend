...
export default function OrdersPage() {
  ...
  // Simulate small random after-hours gaps for demo purposes
    ...
    <main className="min-h-[calc(100vh-3.5rem)] px-4 sm:px-6 lg:px-8 py-8 max-w-screen-xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-0.5">
          Active BUY signals with after-hours gap validation. Gap &gt;5% cancels; 2–5% adjusts entry.
        </p>
      </div>
              ...
              <tbody className="divide-y divide-slate-800/80">
                {orders.map(order => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-slate-400 max-w-[200px] truncate">{order.primary_reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="mt-6 bg-slate-800/30 border border-slate-700/50 rounded-lg px-4 py-3 text-xs text-slate-500">
        <strong className="text-slate-400">Gap thresholds:</strong> After-hours prices are fetched by the backend (Yahoo Finance, ~15-min delay).
        Gap &gt;5% → signal cancelled; Gap 2–5% → entry adjusted +0.2% above AH price; Gap &lt;2% → enter at signal price.
        Connect the backend&apos;s extended_hours module to a live endpoint to enable real-time gap data.
      </div>
    </main>
  );
}
